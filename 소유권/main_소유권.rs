fn main() {
    let s = String::from("hello"); // 변수 s가 범위(main scope) 내에 생성된다.

    takes_ownership(s); // 변수 s의 값이 함수 내로 이동한다.
                        // 그리고 이 시점부터 변수 s는 더 이상 유효하지 않다.

    ///// 이 이후부터 변수 s를 호출하는 경우 컴파일 에러 발생 /////
    ...
    //// 이 이후에 변수 x를 호출 하여도 컴파일 에러가 발생하지 않는다 ////
} // 이 지점에서 변수 x가 범위를 벗어난 후, 다음으로 변수 s도 범위를 벗어난다.
  // 하지만 변수 s의 값은 함수 내로 이동했기 때문에 아무런 일도 일어나지 않는다.

fn takes_ownership(some_string: String) { // some_string 변수가 범위(takes_ownership 메소드 scope) 내에 생성된다.
    println!("{}", some_string);
